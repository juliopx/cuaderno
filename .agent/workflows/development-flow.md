...
- **MANDATORY**: Fill out the PR template in `.github/pull_request_template.md`. Do not leave placeholders.
- **IMPORTANT**: Do not close the Issue or merge the PR without prior validation.

## 3. Mandatory Validation (Golden Rule) ⚠️
...
To limit who can merge and ensure validation, a **Ruleset** (e.g., named `Standard Branch Protection`) must be configured on GitHub (Settings > Rulesets):
